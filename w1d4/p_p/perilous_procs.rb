...
# # filter_out
# # Write a method filter_out that accepts an array and a block as arguments. The method should return a new array where elements of the original 
# array are removed if they return true when given to the block. Solve this using Array#each.
...
# # every?
# # Write a method every? that accepts an array and a block as arguments. The method should return a boolean indicating whether or not all 
...
# # first_index
# # Write a method first_index that accepts an array and a block as arguments. The method should return the index of the first element of the array that 
# returns true when giben to the block. If no element of returns true, then the method should return nil. Solve this using Array#each.
...
# # filter_out!
# # Rewrite your previous filter_out method to mutate the input array instead of returning a new array. That is, write a method filter_out! that accepts 
# an array and a block as args. The method should remove elements of the input array that return true when given to the block. Solve this without using Array.reject!.
...
# # multi_map
# # Write a method multi_map that accepts an array, an optional number (n), and a block as arguments. The method should return a new array where each element 
# of the original array is repeatedly run through the block n times. If the number argument is not passed in, then the the elements should be run through the block once.
...
# # proctition
# # Write a method proctition that accepts an array and a block as arguments. The method should return a new array where the elements that return true when 
# given to the block come before all of the elements that return false when given to the block. This means that the new array has two partitions; the order among elements in the same partition should be the same as their relative order in the input array.
...
# # selected_map!
# # Write a method selected_map! that accepts an array and two procs as arguments. The method should replace the elements that return true when passed 
# into the first proc with their return values when they are passed into the second proc. This method should mutate the input array and return nil.
...
# # proc_suffix
# # Write a method proc_suffix that accepts a sentence and a hash as arguments. The hash contains procs as keys and suffix strings as values. The method 
# should return a new sentence where each word of the original sentence is appended with a suffix if the original word retruns true when given 
# to the corresponding proc key. If an original word returns true for multiple procs, then the suffixes should be appended in the order that they
...
#     arr.each do |word|
#         new_word = word
#         hash.each do |prc_k, add_v|
#             new_word += add_v if prc_k.call(word)
#         end
#         res << new_word
#     end
#     res.join(" ")
...
#     result = Hash.new
#     prcs.each_with_index {|_,i| result[i + 1] = Array.new}
    
#     arr.each do |el|
#         prcs.each_with_index do |prc, j|
#             if prc.call(el)  
#                 result[j+1] << el
#                 break
#             end
...
# The method should return a new sentence where each word of the input sentence is changed by a value proc if the original word returns true when passed into 
...
#     contains_a => reverse               # what! --> what!??? --> ???!tahw
# ) # "he dias ???!tahw"

# p procipher('he said what!',
#     contains_a => reverse,
#     is_yelled => make_question  # what! --> !tahw --> !tahw???
# ) # "he dias !tahw???"
...
# # picky_procipher
# # Write a method picky_procipher that accepts a sentence and a hash as arguments. The hash contains procs as both keys and values. The method should return a 
# new sentence where each word of the input sentence is changed by a value proc if the original word returns true when passed into the key proc. 
...
# def picky_procipher(str, hash) #k_prc, v_prc
#     res = []
#     arr = str.split(" ")
#     arr.each do |word|
#         hash.each do |k_prc, v_prc|
#             if k_prc.call(word)
#                 word = v_prc.call(word)
#                 res << word 
#                 break
#             end            
#         end
...
p picky_procipher('he said what!',
    is_yelled => make_question,
    contains_a => reverse      ##### he is_yell resr
) # "he dias what!???"
